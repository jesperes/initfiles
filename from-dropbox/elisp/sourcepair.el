...
;;; Commentary:
;;
;; This Emacs lisp file provides the function "sourcepair-load" which
;; will load the corresponding header or source file for the current
;; buffer.  For example, if you are looking at the file FooParser.cpp
;; and enter M-x sourcepair-load (or whatever keybinding you've set),
;; the file FooParser.h will be loaded.  It also works the other way
;; as well.  To use it put this file somewhere in your lisp library
;; path and then add something like this to your .emacs file:
;;
;; (load-file "sourcepair.el")
;;
;; KEYBINDINGS:
;;
;; You should set a keybinding to use this function easily.  For
;; example, I add the following to my .emacs file:
;;
...
;; GLOBAL VARIABLES:
;;
;; There are five global variables that can be used to adjust how the
;; function works:
;;
;; sourcepair-source-extensions :
;;
;;    A list containing the recognized extensions for source files.
;;    By default this is set to ( ".cpp" ".cxx" ".cc" ".c" ).  For
;;    example: with the default setting if you are looking at "foo.h",
;;    the function will look for "foo.cpp", "foo.cxx", "foo.cc" or
;;    "foo.c" in that order in the directories specified by
;;    sourcepair-source-path.
;;
;; sourcepair-header-extensions :
;;
;;    A list containing the recognized extensions for header files.
;;    By default this is set to ( ".h" ".hpp" ".hh" ).  For example:
;;    with the default setting if you are looking at "foo.cpp", the
;;    function will look for "foo.h", "foo.hpp" or "foo.hh" in that
;;    order in the directories specified by sourcepair-header-path.
;;    
;; sourcepair-source-path :
;;
;;    A list containing the path's to search for source files.  By
;;    default this is set to ( "." ) which means source files will
;;    only be searched for in the current directory.  Paths that end
;;    in "/*" will be searched recursively.  For example, if you
;;    specified sourcepair-source-path as ( "." "../*" ) the function
;;    will look for source files first in the current directory, and
;;    then in the parent directory, and then in any subdirectories of
...
This variable is used by `sourcepair-load'.  The value should be a
list containing the recognized extensions for source files.  For
example: if the value is ( \".cpp\" \".cxx\" \".cc\" \".C\" \".c\" ),
and you are looking at \"foo.h\", `sourcepair-load' will look for
...
This variable is used by `sourcepair-load'.  The value should be a
list containing the recognized extensions for header files.  For
example: if the value is (\".h\" \".hpp\" \".hh\" ), and you are
looking at \"foo.cpp\", `sourcepair-load' will look for \"foo.h\",
\"foo.hpp\" or \"foo.hh\" in that order in the directories specified
...
This variable is used by `sourcepair-load'.  The value should be a
list containing the directories to search for source files.  By
default this is set to ( \".\" ) which means source files will only be
searched for in the current directory.  Paths that end in \"/*\" will
be searched recursively.  For example, if you specified
`sourcepair-source-path' as ( \".\" \"../*\" ) `sourcepair-load' will
look for source files first in the current directory, and then in the
parent directory, and then in any subdirectories of the parent
directory."
  :type '(repeat string))
...
This is similar to `sourcepair-source-path' except for header files.
See the documentation for `sourcepair-source-path' for more info."
  :type '(repeat string))
...
This variable is used by `sourcepair-load'.  The value should be a
list containing the names of directories to ignore when
`sourcepair-load' is recursively searching subdirectories for header
or source files.  By default this is set to ( \"CVS\" )"
  :type '(repeat string))
...
This function returns t if the filename specified is a C/C++ source
file, or nil otherwise.  Source files are identified by extension via
the variable `sourcepair-source-extensions'."
...
(defun sourcepair-load (arg)
  "Load the corresponding C/C++ header or source file for the current buffer.

This function can be invoked by \\[sourcepair-load].  It will load the
the corresponding header or source file for the current buffer.  For
example, if you are looking at the file FooParser.cpp and press
\\[sourcepair-load], the file FooParser.h will be loaded.  It also
works the other way as well.

If given a prefix argument, the file will be opened in the other
window (find-file-other-window).
  ...
  (catch 'found-matching-file
	...
	(progn
	  (while search-path
		...
		(throw 'found-matching-file 
		       (if arg
			   (find-file-other-window matching-filename)
